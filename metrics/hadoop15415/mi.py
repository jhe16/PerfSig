...
cpu = []
time = []
with open('hadoop15415-cpu.csv') as csv_file:
...
#read time vector
with open('functimevector-15415.pickle', 'rb') as handle:
...
print ('the rank 1 function is: ' + str(names[sortidx[-1]]))
print ('the rank 12 function is: ' + str(names[sortidx[-2]]))
...
ax.plot(timeline, cpuepisode, color = 'b', linewidth=2, label = 'CPU utilization')
# ax.plot(timeline, cpufigure, 'r-', linewidth=2, label = 'Filtered CPU utilization')
# ax.plot(timeline, cpuslope, color = 'r')
# ax.plot(timeline, causefuncfigure, color = 'r')
# plt.axvline(x=cutstart, linestyle='--')
plt.axvline(x=51, linestyle='--', linewidth=5, label = 'bug start time')
...
# ax.legend(['root cause function (hadoop-15415)'])
